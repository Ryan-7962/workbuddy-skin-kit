...
## 主题配色规范（硬约束，新增或修改主题必读）

### 为什么必须成文

换肤的本质是：把**颜色不可控的壁纸**映射成**必须可读的 UI 配色**。壁纸是任意的，而可读性是可量化的硬约束。凭肉眼判断配色"看着还行"必然漏——本项目前 5 个报障里有 2 个就是配色问题，且都是"看着能跑、实际不可读"。

### 一、四条硬约束（用 `tools/audit_contrast.mjs` 校验）

| 项目 | 阈值 | 说明 |
|---|---|---|
| 正文 × 底色 | **≥ 4.5** | WCAG AA 正文标准 |
| 块内文字（代码块/卡片上的字） | **≥ 4.5** | 同上 |
| 强调色（链接/按钮）× 底色 | **≥ 3** | 按 `--wb-accent-readable` 判定，不是品牌原色 |
| 内容块底色 × 面板底色 | **≥ 1.06** | 块与底必须有层次，否则"糊成一片" |

**当前基线**：16 个主题全达标，最差项 3.40（miku-488137 的强调色）。**新增主题不得低于这个水平。**
...
| 面板底 | `var(--wb-surface)` |
| 正文 | `var(--wb-text)` |
| 次级文字 | `color-mix(in srgb, var(--wb-text) 70%, transparent)` |
| 内容块 / 卡片 / 输入框底 | `color-mix(in srgb, var(--wb-surface) 86~93%, var(--wb-text))` |
| 链接 / 按钮底 | `var(--wb-accent-readable)` = `color-mix(in srgb, var(--wb-accent) 64%, var(--wb-text))` |
| 按钮前景 | `var(--wb-surface)`（与按钮底互为反白） |
| 描边 | `color-mix(in srgb, var(--wb-accent) 30~45%, transparent)` |

**核心技巧：所有派生色都混入 `--wb-text`，方向由主题自动决定。**
浅色主题 text 是深色 → 派生色被压深；深色主题 text 是浅色 → 派生色被提亮。**一套规则适配两种模式，永远不要写两份。**

**为什么强调色必须混 text**：品牌色直接当链接/按钮色时，浅色主题下实测对比度只有 **1.85~1.99**（阈值 3），等于看不见。混入 64% text 后升到 3.40~5.79，且仍保留品牌色倾向。64% 这个比例是实测出来的——72% 时 miku-488137 是 2.94，差 0.06 不达标。

### 三、从壁纸推导 surface（新增主题时按此顺序）

1. 取壁纸的**平均亮度**（不是主色，是亮度——它决定整体明暗）
2. 亮壁纸 → 浅色 surface：近白，可带极淡的主色染色
3. 暗壁纸 → 深色 surface：近黑，可带主色染色
4. **text 取 surface 的反向极值**（浅底配深字、深底配浅字），**不要用中间灰** —— 灰字在花背景上最先阵亡
5. 跑 `audit_contrast.mjs`，不达标就调 surface/text，不要靠调遮罩强度去掩盖

### 四、每次改动的完成标准（三项全过才算完）

```bash
node tools/audit_contrast.mjs <主题目录>   # 配色对比度
node tools/audit_routes.mjs                # 各功能页容器
node tools/diag_skin_css.mjs <端口> <截图>  # 锚点有效性 + 真实渲染截图
```

**最后一项必须人眼看过截图**——状态字段全绿不等于渲染正确，这是踩过坑的。
...
| 🎨 按钮**看得见却点不动**（`elementFromPoint` 命中测试显示命中的就是按钮，样式也全正常） | 按钮落在顶栏 `-webkit-app-region: drag` 区域内。Electron 的拖拽区是**几何区域**，落在其中的鼠标事件在浏览器进程就被当成"拖动标题栏"消费掉，渲染进程根本收不到 click | 已修：注入时扫描 drag 区域算出安全落点（顶栏底边 + 10px）作为默认位；拖拽下界与 resize 夹取同步使用该安全线。**关键：CDP 的 `Input.dispatchMouseEvent` 绕过该拦截，所以"程序点击成功"不能证明真人可达**——必须用几何断言：`tools/diag_skin_click.mjs`（按钮与 drag 区域重叠面积须为 0）。换版本后跑 `tools/audit_drag_band.mjs` 逐页核验带高 |
...
| 主题面板底部几个主题够不到 | 面板自按钮下方起排，主题多于窗口高度时被裁切，且不能滚动 | 已修：展开前按"按钮下方可用空间"设 `max-height`（最小 140px）并允许纵向滚动。**约束：浮动面板的高度必须由可用空间推导，不能写死**，否则按钮位置一变就会重新溢出 |
